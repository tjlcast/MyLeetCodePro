...
public class _435_Non_overlapping_Intervals {
	...
	public static void main(String[] args) {
		...
		Interval[] intervals = new Interval[datas.length];

		for (int i = 0; i < intervals.length; i++) {
			intervals[i] = new Interval(datas[i][0], datas[i][1]);
		}

		int eraseOverlapIntervals = new _435_Non_overlapping_Intervals().work(intervals);
		System.out.println(eraseOverlapIntervals);
	}

	public int work(Interval[] intervals) {
		return new Solution().eraseOverlapIntervals(intervals);
	}

	class Solution {
		public int eraseOverlapIntervals(Interval[] intervals) {
			Arrays.sort(intervals,
					(Interval x, Interval y) -> (x.start == y.start) ? (x.end - y.end) : (x.start - y.start));
			int count = 0;
			if (intervals.length <= 1) return 0 ;
			
			int terminate = intervals[0].end;
			for (int i = 1; i < intervals.length; i++) {
				if (terminate <= intervals[i].start) {
					terminate = intervals[i].end ;
				} else if (terminate <= intervals[i].end) {
					count++ ;
				}  else {
					terminate = Math.min(terminate, intervals[i].end) ;
					count++ ;
				}
			}
