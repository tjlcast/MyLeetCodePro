...
public class Test {
    ...
    public static void main(String[] args) throws NotFoundException, CannotCompileException, InvocationTargetException, NoSuchMethodException, InstantiationException, IOException, IllegalAccessException {
        createProxy();
    }
     ...
     * 动态创建字节码
     */
    private static void createProxy() throws NotFoundException, CannotCompileException, NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException, IOException {

        ClassPool pool = ClassPool.getDefault();
        CtClass cc = pool.makeClass("com.tjlcast.clazz_byte_operation.javassist.sample.StationProxy");

        // 设置接口
        CtClass interface1 = pool.get("com.tjlcast.clazz_byte_operation.javassist.sample.TicketService");
        cc.setInterfaces(new CtClass[] { interface1 });

        // 设置Field
        CtField field = CtField.make("private com.tjlcast.clazz_byte_operation.javassist.sample.Station station;", cc);
        cc.addField(field);

        // 配置构造函数
        CtClass stationClass = pool.get("com.tjlcast.clazz_byte_operation.javassist.sample.Station");
        ...
        sellTicket.setBody("{this.showAlertInfo(\"××××您正在使用车票代售点进行购票，每张票将会收取5元手续费！××××\");" + "station.sellTicket();"
                + "this.takeHandlingFee();" + "this.showAlertInfo(\"××××欢迎您的光临，再见！××××\");}");
